// Archivo ControladorJuego java del paquete controlador
...
public class ControladorJuego {
    ...
    // Controlan el menu y el estado del juego
    ...
    // Hilo encargado de mover los fantasmas automaticamente
    private HiloFantasma hiloFantasma;

    // Controla que opcion del menu inicial esta seleccionada
    // 0 Iniciar partida
    // 1 Salir
    private int opcionMenu;
    
    // Evita iniciar varias cuentas regresivas al mismo tiempo
    ...
    public ControladorJuego() {
        ...
        // Crear Pac Man en su posicion inicial
        ...
        // Crear ventana y conectar las opciones del menu superior
        ...
        // El teclado controla el menu inicial y luego a Pac Man
        ...
        // Crear el hilo de fantasmas pero todavia no se inicia
    ...
    public void iniciar() {
        ...
        while (true) {
            ...
            // Reanudar fantasmas mientras el juego este activo
            hiloFantasma.setPausado(false);

            // Mover Pac Man
            pacman.mover(mapa.getMatriz());

            // Ya no se mueven fantasmas aqui
    ...
    private void verificarComida() {
        ...
        // Revisa si Pac Man esta sobre una comida
        ...
        // Verifica si Pac Man esta sobre una vida extra
        if (mapa.hayVidaExtra(pacman.getY(), pacman.getX())) {

            // Pac Man gana una vida
            pacman.ganarVida();

            // Se elimina la vida extra del mapa
            mapa.quitarPoder(pacman.getY(), pacman.getX());

            System.out.println("Vida extra obtenida");
        }
        // Verifica si Pac Man comio una super bolita
        if (mapa.haySuperBolita(pacman.getY(), pacman.getX())) {

            pacman.activarSuperPoder();
    ...
    // Recorre todos los fantasmas del juego
    for (int i = 0; i < fantasmas.length; i++) {

        // Verifica si algun fantasma ocupa la misma posicion que Pac Man
        if (fantasmas[i].getX() == pacman.getX() &&
            fantasmas[i].getY() == pacman.getY()) {

            // Si Pac Man tiene activo el super poder
            // puede comerse al fantasma
            if (pacman.tieneSuperPoder()) {

                // Se suman puntos adicionales por comer al fantasma
                pacman.sumarPuntos(200);

                // El fantasma regresa a la posicion inicial
                // la casa de los fantasmas
                fantasmas[i].reiniciarPosicion(10, 10);

                System.out.println("Fantasma comido");

            } else {

                // Si Pac Man no tiene el super poder
                // pierde una vida al ser atrapado
                pacman.perderVida();
                ...
                if (pacman.estaMuerto()) {
                    ...
                    // Se marca el juego como perdido
                    juegoPerdido = true;

                    // Se le avisa al panel para mostrar GAME OVER
                    panel.setJuegoPerdido(true);

                    // Guarda el High Score si el puntaje actual lo supera
                    guardarHighScoreSiEsNecesario();
                    ...
                    // Si aun quedan vidas
                    // Pac Man vuelve a la posicion inicial
                    pacman.reiniciarPosicion();

                    // Todos los fantasmas regresan a sus posiciones iniciales
            ...
            // Se sale del ciclo para evitar procesar mas colisiones
            // durante esta actualizacion del juego
            break;
        }
    }
}
    ...
    private void iniciarPartida() {
        ...
        if (iniciandoPartida || partidaIniciada) {
            return;
        }
        ...
        new Thread(() -> {
            ...
            try {
                panel.setCuentaRegresiva(3);
                panel.repaint();
                Thread.sleep(1000);

                panel.setCuentaRegresiva(2);
                panel.repaint();
                Thread.sleep(1000);

                panel.setCuentaRegresiva(1);
                panel.repaint();
                Thread.sleep(1000);

                panel.setCuentaRegresiva(0);
                panel.repaint();
                Thread.sleep(700);
    ...
    private void pausarReanudar() {
        ...
        if (partidaIniciada) {
            ...
            // Pausa o reanuda tambien el hilo de fantasmas
