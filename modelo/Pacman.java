// Archivo Pac Man java del paquete modelo
// Este archivo representa datos reglas y comportamiento interno del juego

package com.mycompany.pacman.modelo;

public class Pacman extends Personaje {

    private int vidas;
    private int puntaje;
    // Indica si Pac Man puede comer fantasmas
    private boolean superPoder = false;

    // Tiempo restante del super poder
    private int tiempoSuperPoder = 0;

    public Pacman(int x, int y) {
        super(x, y, 1);
        this.vidas = 3;
        this.puntaje = 0;
        this.direccionActual = "DERECHA";
        this.direccionDeseada = "DERECHA";
    }

    @Override
    public void mover(int[][] mapa) {

        // Intenta cambiar a la direccion deseada
        if (puedeMover(mapa, direccionDeseada)) {
            direccionActual = direccionDeseada;
        }

        // Si puede moverse en la direccion actual avanza
        if (puedeMover(mapa, direccionActual)) {
            switch (direccionActual) {
                case "ARRIBA":
                    y--;
                    break;
                case "ABAJO":
                    y++;
                    break;
                case "IZQUIERDA":
                    x--;
                    break;
                case "DERECHA":
                    x++;
                    break;
            }
        }
    }

    private boolean puedeMover(int[][] mapa, String direccion) {

        int nuevaX = x;
        int nuevaY = y;

        switch (direccion) {
            case "ARRIBA":
                nuevaY--;
                break;
            case "ABAJO":
                nuevaY++;
                break;
            case "IZQUIERDA":
                nuevaX--;
                break;
            case "DERECHA":
                nuevaX++;
                break;
        }

        // Validar limites del mapa
        if (nuevaY < 0 || nuevaY >= mapa.length ||
            nuevaX < 0 || nuevaX >= mapa[0].length) {
            return false;
        }

        // Validar que no sea pared
        return mapa[nuevaY][nuevaX] != Mapa.PARED;
    }

    // Suma puntos al puntaje actual
    public void sumarPuntos(int puntos) {
        this.puntaje += puntos;
    }

    // Resta una vida cuando un fantasma atrapa a Pac Man
    public void perderVida() {
        this.vidas--;
    }
    
    // Suma una vida a Pac Man
    public void ganarVida() {
        vidas++;
    }
    // Activa el super poder
    public void activarSuperPoder() {

        superPoder = true;

        // dura 10 segundos
        tiempoSuperPoder = 10;
    }
    
    // Reduce el tiempo del poder
    public void actualizarSuperPoder() {

        if (superPoder) {

            tiempoSuperPoder--;

            if (tiempoSuperPoder <= 0) {

                superPoder = false;

                tiempoSuperPoder = 0;
            }
        }
    }
    
    public boolean tieneSuperPoder() {
        return superPoder;
    }
    
    public int getTiempoSuperPoder() {
        return tiempoSuperPoder;
    }

    // Reinicia la posicion de Pac Man al punto de inicio
    // Se utiliza cuando pierde una vida
    public void reiniciarPosicion() {
        x = 1;
        y = 1;

        direccionActual = "DERECHA";
        direccionDeseada = "DERECHA";
    }

    // Verifica si Pac Man ya no tiene vidas
    public boolean estaMuerto() {
        return vidas <= 0;
    }

    public int getVidas() {
        return vidas;
    }

    public int getPuntaje() {
        return puntaje;
    }
}
